...
* `形象理解`
```
* 使用栈就好像我们去饭馆里吃饭一样,我们只管点菜(发出申请)、付钱和吃(使用),吃饱了就走,而不必例会切菜、洗菜等准备工作
和洗碗、刷锅等扫尾工作,好处是快捷、但自由度小
* 使用堆就好像自己动手做喜欢的菜式,但比较麻烦,但是符合自己的口味、而且自由度大
```
* `理解栈和堆在程序编译运行过程的作用`
```
// main.cpp
...
* `浅拷贝和深拷贝`
```
* 只有在结构体成员为指针才会有浅拷贝和深拷贝
* 深拷贝和浅拷贝：在有指针的情况下，浅拷贝只是增加了一个指针指向已经存在的内存，
